    ...
    case "fim-margin": {
      const rows = await all(
        ...
        `SELECT dept_code, dept_name, report_date, net_sales_zar, pos_margin_pct, operating_margin_pct,
         ...
         FROM fim_daily WHERE dept_code != 'TOTAL' AND report_type != 'weekly_freshb' AND report_date=(SELECT MAX(report_date) FROM fim_daily WHERE report_type != 'weekly_freshb') ORDER BY net_sales_zar DESC`,
      );
      const aoa: unknown[][] = [
